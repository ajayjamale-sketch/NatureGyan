...
import {
  ...
  ChevronRight, Globe, Megaphone, Leaf, X
} from 'lucide-react';
...
export default function NGODashboard() {
  ...
  const handleLaunchCampaignSubmit = (e: React.FormEvent) => {
    ...
    const newCamp = {
      ...
      status: 'active',
      volunteers: 1,
      target: parseInt(campTarget) || 5000,
      achieved: 0,
    ...
    toast.success('Campaign deployed! Live for citizen volunteers.');
  };
    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap">
          <Button variant="outline" className="h-9 text-sm border-border text-foreground hover:bg-muted font-medium" onClick={() => navigate('/dashboard/volunteers')}>
            <Users className="w-4 h-4 mr-2" /> Manage Volunteers
          </Button>
          <Button className="gradient-primary text-white h-9 text-sm font-semibold" onClick={() => setAddCampaignModal(true)}>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-xl p-5 bg-card border border-border">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button className="gradient-primary text-white h-8 text-xs font-semibold" onClick={() => setAddCampaignModal(true)}>
              ...
              {state.campaigns.slice(0, 3).map(c => (
                <div key={c.id} className="p-4 rounded-lg bg-muted/50 border border-border hover:border-primary/30 transition-colors">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <button onClick={() => handleShare(c.name)} className="p-1.5 rounded-lg border border-border hover:border-primary/40 text-muted-foreground hover:text-primary transition-colors">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl p-5 bg-card border border-border">
            <div className="flex items-center justify-between mb-4">
              ...
              <button className="text-xs text-primary hover:underline font-semibold" onClick={() => navigate('/dashboard/volunteers')}>Manage</button>
            </div>
